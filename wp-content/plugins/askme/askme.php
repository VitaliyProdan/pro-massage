...
/**
 * Plugin Name: Ask Me
 * Plugin URI: http://kilo-moana.com/ask-me-wordpress-plugin/
...
function create_askme_tags() {
    ...
    register_taxonomy( 'asktag', 'askme', $args );

}

add_action('init', 'create_askme_tags');
...
function show_asks(  ) {
    ...
    <div id="askme">
        <form id="newask" name="newask" method="post" action="">
            ...
            <label for="question" id="questionLabel"><?php _e('Question to ask', 'askme-plugin'); ?></label><br />
            ...
            <p><input type="submit" value="Спросить" tabindex="6" id="submit" name="submit" /></p>
...
function askme_settings_init() {
    ...
    add_settings_field(
        ...
        __('Number of Askmes', 'askme-plugin'),
...
function askme_number_callback() {
    echo '<input name="askme_setting_number_askmes" id="gv_thumbnails_insert_into_excerpt" type="text" class="code" value="' . get_option( 'askme_setting_number_askmes', 5 ) . '" />
        <p class="description">' . __('How much askmes you want to display on one page.', 'askme-plugin') . "</p>";
}
